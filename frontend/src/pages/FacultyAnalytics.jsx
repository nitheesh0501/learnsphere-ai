...
import { Users, AlertCircle, CheckCircle2, Send, Search, Shield, Sparkles, RefreshCw } from 'lucide-react';
...
export default function FacultyAnalytics({ addToast }) {
  ...
  useEffect(() => {
    // Attempt fetching real backend data if available
  ...
  const handleIntervention = async (studentId, action) => {
    ...
    // Call Flask API endpoint (with fallback handling)
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto px-2 sm:px-4">
      ...
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 p-5 sm:p-6 rounded-2xl border border-slate-800 text-white shadow-xl">
        <div>
      ...
      {/* KPI Overview Cards (Mobile Landscape Ready) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 flex items-center justify-between shadow-sm">
          <div>
            ...
            <p className="text-3xl font-black text-rose-600 mt-1">{atRiskCount}</p>
            <p className="text-[11px] text-rose-600 font-bold mt-1">Requires Early Intervention</p>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-6">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full md:w-auto">
            ...
            <div className="relative flex-1 sm:w-64">
              ...
              <input
                ...
                className="w-full bg-slate-50 border border-slate-200 rounded-xl pl-9 pr-3 py-1.5 text-xs text-slate-900 font-semibold focus:outline-none focus:border-blue-600"
              />
            </div>
              ...
              {['All', 'Flagged', 'Nudge Sent', 'Resolved'].map((st) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-lg transition-all whitespace-nowrap ${
                    filterStatus === st ? 'bg-blue-600 text-white shadow-xs' : 'text-slate-600 hover:text-slate-900'
                  }`}
                >
                  {st}
                </button>
        ...
        {/* Roster Table (Scrollable & Responsive for Mobile/Landscape) */}
                ...
                <tr key={stu.id} className="hover:bg-slate-50/80 transition-colors">
                  ...
                  <td className="py-3.5 px-3">
                    <div className="flex items-center space-x-3">
                      <div className="w-8 h-8 rounded-lg bg-blue-100 text-blue-700 font-bold flex items-center justify-center text-xs border border-blue-200 shrink-0">
                        {stu.avatar}
                      </div>
                      <div>
                    ...
                    <div className="flex items-center justify-end space-x-2">
                      {stu.status !== 'Resolved' && (
                        <button
                          ...
                          className="px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold transition-all shadow-xs flex items-center space-x-1 active:scale-95"
                        >
